// 주행 경로 기기 시험. 사람 손 없이 돈다.
//
// 진단 경로만 자동화해 두면 확인된 것은 진단이고 제품은 아니다. 러너가 쓰는 길은
// 주행 화면이므로 그 길도 같은 방식으로 확인한다.
//
...
console.log('주행 표식을 넣고 앱을 띄웁니다 (시작 버튼을 누르지 않습니다)');
...
if (!trace.some(function (m) { return /주행 시작/.test(m.msg); })) {
  const why = trace.find(function (m) { return m.kind === 'err'; });
  fail('주행이 시작되지 않았습니다.' + (why ? ' 기록: ' + why.msg : ' 기록이 비었습니다')
...
// 앱을 다시 띄워 주행을 정리한다. 남기면 iOS 가 계속 깨운다
await launch(dev, BUNDLE);
await sleep(3000);

if (judged.fixes.length === 0) {
  fail('배경으로 밀린 동안 주행이 위치를 한 건도 받지 못했습니다.\n'
    + '      진단은 통과하는데 주행이 못 받는다면 분배가 잘못된 것입니다');
}
if (judged.errors.length) fail('오류가 있습니다. 위 기록을 보세요');

console.log('\n통과. 주행이 배경에서 위치 ' + judged.fixes.length + '건을 받았습니다');
