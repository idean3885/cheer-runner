...
['course.json', 'courses.json', 'runs.jsonl', 'trace.jsonl', 'session.json',
  'auto-run', 'auto-start', 'auto-course', 'auto-records', 'auto-record-detail']
...
fs.writeFileSync(path.join(docs, 'runs.jsonl'), [
  ...
  JSON.stringify({
    ...
    spots: [
      { lat: north(60), lon: LON, rad: 30 },
      { lat: north(120), lon: LON, rad: 30 }
    ],
    splits: [
...
// 6. 기록 상세. 경로·지점 핀·구간 목록이 한 화면에 있는지 본다
fs.writeFileSync(path.join(docs, 'auto-record-detail'), '');
await run('xcrun', ['simctl', 'terminate', dev, BUNDLE], { allowFail: true });
await run('xcrun', ['simctl', 'launch', dev, BUNDLE]);
await sleep(7000);
console.log('기록 상세 화면');
await shot(dev, '6-record-detail');

// 7. 위치를 받지 못하는 화면. 지하에서 달리기를 눌렀을 때의 자리다.
...
await shot(dev, '7-blocked');
await run('xcrun', ['simctl', 'privacy', dev, 'grant', 'location-always', BUNDLE], { allowFail: true });

if (!(await alive(dev))) fail('앱이 죽었습니다. 그림만으로는 알기 어려운 상태입니다');

console.log('\n그림 ' + 7 + '장을 만들었습니다. ' + outDir);
...
console.log('        기록 목록이 날짜·거리·시간·평균 페이스를 적는가 · 상세에 핀과 구간이 함께 있는가');
