...
|---|---|---|---|---|
| 모바일앱 | Expo React Native | 배경에서 위치를 받고 응원한다 | 스토어와 기기 | 기기 저장, 위치 서비스, 음성 합성, 지도 이미지 서비스 |
| 기기 저장 | 기기 내 저장 | 코스, 보관함, 궤적, 진단 기록 | 기기 | 없음 |

러너가 직접 쓰는 것은 모바일앱 하나다. 기기 저장은 그 뒤에 있다.

**배포되는 것이 하나다.** 서버가 없어서가 아니라 이 도메인에 서버가 필요하지 않기 때문이다. 코스는 러너 한 사람의 것이고 남과 견주는 일은 다른 컨텍스트다.

### 웹 프로토타입을 걷었다

컨테이너가 둘이던 시기가 있었다. 앱을 만드는 동안 로직 확인을 웹에서 했고, 그래서 **같은 도메인 규칙이 앱과 웹에 각각 구현된 상태**였다. 이 그림이 드러내려던 문제가 그것이었다.

정리 시점을 「앱이 안정된 뒤」가 아니라 **「앱 쪽 시험이 같은 표본으로 같은 판정을 낼 때」** 로 뒀고, 그 조건으로 판정해서 걷었다. 웹 시험 9건 중 7건은 앱 시험이 같은 판정을 낸다. 남은 2건은 웹에서만 성립하는 물음이다.

| 웹에만 있던 판정 | 왜 옮기지 않나 |
|---|---|
| 트랙 점이 4원소여도 지도가 터지지 않는가 | 웹 쪽 자료 구조 문제다. 앱에 그 형태가 없다 |
| 종료 시 오디오·미디어세션을 남기지 않는가 | 웹에서 실행 맥락을 유지하려던 우회책이다. [DOMAIN.md](DOMAIN.md) 「옮기지 않는 것」에 있다 |

웹에서 나온 측정값은 코드가 아니라 계약과 이 문서에 남아 있다. 배경 위치 불가는 [ADR 0001](adr/0001-cross-platform-choice.md), 좌표 차분만 쓰면 거리가 15% 짧은 것과 652초 단절이 창 페이스를 망가뜨린 것은 `DOMAIN.md`, 표시가 수집 경로 안에 있어 1.34km 에서 끊긴 것은 이 문서의 패턴 표, 플랫폼 호출 42개가 창구 넷으로 수렴한 것은 아래 포트 절이다.

**한쪽만 고치는 실수는 이미 시작돼 있었다.** 걷기 직전 상태에서 앱에만 있던 것이 코스 보관함, 시작 조건 가드, 알림음, 이탈 판정, 페이스 표본 하한이었고 웹은 이전 낱말을 쓰고 있었다. 사고로 보이지 않은 이유는 아무도 웹을 고치지 않았기 때문이고, 그 상태를 더 두면 다음에 규칙을 고칠 때 어느 쪽이 정본인지 사람이 판단해야 한다.
